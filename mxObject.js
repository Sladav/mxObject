  ...
  mxObject.prototype = {
    ...
    */
    getMixinsDeep(_flattenedTree = []){
      this.__mixins__.map( v => {
        ...
        if(v.__type__ === 'mxObject') v.getMixinsDeep(_flattenedTree)
          })
    ...
    */
    getMixinsBroad(_flattenedTree = [], _depth = 0){
      function helper(){
        if(typeof _flattenedTree[_depth] === 'undefined') _flattenedTree[_depth] = [];
        this.__mixins__.map( v => {
          _flattenedTree[_depth].push(v)
          if(v.__type__ === 'mxObject') v.getMixinsBroad(_flattenedTree,_depth+1)
        })
        return _flattenedTree;
      }
